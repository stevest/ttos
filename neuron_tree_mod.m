...
function [tname path] = neuron_tree_mod (intree, tname, res, options)
% Custom region names (STEFANOS)
uRn = {'soma','axon','dend','apic'};
% Custon region list names (STEFANOS)
rlnames = {'somatic','axonal','basal','apical'};
...
switch format
    case '.nrn' % save file in the NEURON .nrn format
        ...
        if luR > 1,
            for te = 1 : luR,
                fwrite (neuron, ['create ' uRn{te} '[' num2str(H1 (te)) ']', ...
        ...
        for ward = 3 : N,
            ...
            waitbar (ward / N, HW);
            if luR > 1,
                fwrite (neuron, [num2str(tree.R (idpar (ward))) '['], 'char');
            else
        ...
        for te = 1 : luR,
            ...
            if luR > 1,
                fwrite (neuron, ['      ' name '_' uRn{te} '[ward]{', char(13), char(10)], 'char');
            else
        ...
        for te = 1 : luR,
            ...
            HW = waitbar (0, ['writing cylinders of region' uRn{te} ' ...']);
            set (HW, 'Name', '..PLEASE..WAIT..YEAH..');
            for ward = 1 : H1 (te),
